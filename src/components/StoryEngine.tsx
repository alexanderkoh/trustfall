...
import SFXPlayer, { SFXUtils } from './SFXPlayer';
...
export default function StoryEngine() {
  ...
  const handleSkipSlide = () => {
    SFXUtils.playSlideTransition();
    handleNextSlide();
  };

  const handleSkipAll = () => {
    SFXUtils.playProtocolStart();
    router.push('/faction-select');
  };
  ...
  return (
    <div className="fixed inset-0 overflow-hidden">
      ...
      <SFXPlayer volume={0.3} enabled={!isMuted} />
        ...
        >
          ...
          <div className="absolute top-8 left-8 z-40">
            <motion.button
              ...
              className="bg-black/60 rounded-lg p-3 hover:bg-black/80 transition-colors"
          ...
          <AnimatePresence>
            {showControls && (
              ...
              >
                <div className="bg-black/80 retro-border border-green-400 rounded-lg p-4 space-y-3">
                  {/* Mute Button */}
                  <motion.button
                    onClick={handleToggleMute}
                    className={`w-full retro-border ${isMuted ? 'border-red-400' : 'border-green-400'} rounded px-4 py-2 font-pixel text-xs transition-colors ${
                      isMuted ? 'text-red-400 hover:bg-red-900/20' : 'text-green-400 hover:bg-green-900/20'
                    }`}
                  ...
                  <motion.button
                    ...
                    className="w-full retro-border border-yellow-400 rounded px-4 py-2 font-pixel text-xs text-yellow-400 hover:bg-yellow-900/20 transition-colors"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    SKIP SLIDE
                  ...
                  <motion.button
                    ...
                    className="w-full retro-border border-purple-400 rounded px-4 py-2 font-pixel text-xs text-purple-400 hover:bg-purple-900/20 transition-colors"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    SKIP ALL
